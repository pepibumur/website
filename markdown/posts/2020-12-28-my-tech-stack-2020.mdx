...
I guess because we are approaching the end if this so-odd year.
Therefore,
I'd like to share with you what has been my preferred tech stack in 2020,
and that most likely continue to be in 2021.

## [React](https://reactjs.org) for building web frontends

I really like React's approach for building declarative UIs.
...
I haven't tried [Vue](https://vuejs.org) myself therefore I can't say much about it.
I'm not a huge fan of Facebook steering the framework,
but I'm optimistic it'll become _(if it hasn't done already)_ a community-driven project.
...
The introduction of dynamic behaviours resulted in writing imperative Javascript that called DOM APIs.
I wanted to use React's declarative approach and GatsbyJS allowed that.
When I came across there were a bunch of concepts that I had to learn.
For example, its approach to decouple the UI from the data sources using a GraphQL API.
The learning curve was a bit seep,
...
They take care of providing an environment where my Gatsby websites can be built,
...
I might be a bit biased here because I work at [Shopify](https://shopify.com),
but I really enjoy building web services with Ruby on Rails.
I can confirm both, the programming language and the framework sparks joy when using it.
I'm not an expert in any on both,
but I reached a point where I feel fluent and I can't have something up and running quickly.
What I also like about Rails and Ruby is that there's less fatigue compared to Javascript.
If you need a database ORM, [ActiveRecord](https://guides.rubyonrails.org/active_record_basics.html) is there to help.
If you need to expose a GraphQL API, you can use [GraphQL Ruby](https://graphql-ruby.org).

Theese are the Gems that I usually add to projects:
...
I'm not an infrastructure person and therefore I appreciate services that take care of putting my code into production.
...
and in a matter of minutes I have a Rails app up and running on production.
Through addons I can add a [PostgreSQL](https://postgresql.org) database,
and a Redis instance that I can point Sidekiq to.

As a side note,
I have to say I really like the `run` feature which allows me to open a Rails console with the production instance and debug issues in production:
...
What I like about it is that it's integrated into GitHub's UI,
the user interface is fluid compared to other services that I've used in the past,
and it's very easy to reuse CI logic across projects by defining actions.
It took long for GitHub to step into the business,
but I have to say they beat the market with a extremely high-quality product.
...
Moreover, classes delimit properties such as a color and margin to a pre-defined set of values.
As a result, UIs look more consistent and harmonious.

After coming across the framework,
I followed the authors of it,
x and y,
and I've been a huge fan of the work that they've been doing.
...
Moreover,
its dynamism makes it very suitable for experimenting ideas and workflows.
...
I love VSCode.
It's a masterpiece from Microsoft.
I can use it with multiple programming languages like Ruby and Typescript,
extend it through community-built extensions,
and configure it through workspace settings.
Even though it's built with [Electron](https://www.google.com/search?client=safari&rls=en&q=electronjs&ie=UTF-8&oe=UTF-8),
and that inevitably means using more computer resources than other editors that are built natively,
I haven't tried any other editor that is as close as great as VSCode is.
Between developer experience and efficient usage of computer resources,
I lean more towards the former in this domain.

## [React Native](https://reactnative.dev) for building apps

I really like Swift and Apple's direction with [SwiftUI](https://developer.apple.com/xcode/swiftui/),
...
you just need to make the technology an implementation detail and focus on the product.
In the recent years I've shifted from treating technology as a goal,
to using it as a mean to build tools that solve users' problems.
To achieve this,
...
don't forget to tag me ([@pedropbuendia](https://twitter.com/pedropbuendia));
