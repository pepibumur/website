...
- [Cocoapods](https://craftweg.com/module-caching-in-xcode-projects/cocoapods.org): Like CocoaPods, Tuist represents your project as a dependency tree, which gets translated into a set of Xcode projects and workspaces. The main difference is that CocoaPods is in control of the structure of that graph and how it integrates into an existing project, and with Tuist, the structure of the graph is defined by the user using the [dependencies API](https://tuist.io/docs/dependencies/local/). A downside of this set up for integrating third-party dependencies is that clean builds have to compile those dependencies too (source code), and that motivated the building of Carthage. From CocoaPods, Tuist gets the idea of having a dependency graph, and generating Xcode projects and workspaces ready to be built.
...
![An image that shows the setup that companies adopt when using Buck or Bazel](images/module-caching-1.png)
...
![An image that shows how caching works with Tuist](images/module-caching-2.png)
